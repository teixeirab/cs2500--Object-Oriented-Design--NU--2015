 ...
 * Jesse_model.IntReader} uses to determine if the input integer is valid and to produce
...
public interface IntReader {
   ...
   * {@link Jesse_model.IntReader.IntValidator#apply(int)} method should return {@code null} if the
   ...
   * move is validated using the {@link Jesse_model.IntReader.IntValidator} object, which either
   ...
   * @return the move
   * @throws java.io.IOException for I/O problems with the underlying input and
   * output streams
   * @throws java.util.NoSuchElementException if the input stream ends
   */
  int nextInt(String prompt, IntValidator validator) throws IOException;

  /**
   * Constructs an {@link Jesse_model.IntReader} that uses the given input and output
   ...
   * @return the new {@code Jesse_model.IntReader}
   */
