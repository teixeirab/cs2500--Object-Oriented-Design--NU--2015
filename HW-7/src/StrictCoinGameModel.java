...
public class StrictCoinGameModel implements CoinGameModel {
  ...
  /*Invariant: list of integers cannot
  be empty and must contain at least two players */

  private ArrayList<Boolean> board = new ArrayList<Boolean>();


  /* Invariant: board is at least size 1 and only contains O and - */

  private int currentTurn = 0; /* Invariant: currentTurn must be equal or greater than 0 and cannot
  be
   ...
   * @param players is an array that represents the names of each player
   * For example int 1 represents Player 1 and so on. PlayerIds.size() > 0
   * currentTurn >= 0 < (playerId.size() - 1)
  ...
  public void addPlayer(int location, String name) {
    ...
    if (location < 0 || location > playerIds.size()) {
